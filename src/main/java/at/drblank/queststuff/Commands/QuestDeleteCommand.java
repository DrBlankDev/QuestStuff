...
public class QuestDeleteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission(QuestStuff.getInstance().getConfig().getString("QuestPermissions.deletequest"))) {
        ...
        if (args.length <= 0) {
            player.sendMessage(QuestStuff.getInstance().getConfig().getString("Errors.syntax") + label
                    + QuestStuff.getInstance().getConfig().getString("QuestNotifications.questdeletesyntax"));
            return true;
        }
